...
async def get_offers(query: OfferRequest = Query()) -> dict:
    ...
    # Time
    filters.append(f"start_date >= {query.timeRangeStart // 1000}")
    filters.append(f"end_date <= {query.timeRangeEnd // 1000}")

    # Days
    filters.append(f"(end_date - start_date) >= EXCTRACT(EPOCH FROM (INTERVAL '{query.numberDays} days'))")
